...
cd /tmp/FEOTS
autoreconf --install
FCFLAGS="-Ofast -ffree-line-length-none" ./configure --enable-mpi --prefix=${INSTALL_ROOT}/feots
make
make install
...
mkdir -p ${INSTALL_ROOT}/feots-db/argentine_basin
gsutil -m cp -r gs://feots-db/E3SMV0-HILAT-5DAVG ${INSTALL_ROOT}/feots-db
gsutil -m cp -r gs://feots-db/argentine_basin_regional_operators/* ${INSTALL_ROOT}/feots-db/argentine_basin/
